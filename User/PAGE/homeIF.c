...
static const struct HearStr IFHear[IFNUM]=
{
    ...
    {"ÔÆÆ½Ì¨²ÎÊý:","CLOUD PLATFORM:"},   //ÔÆÆ½Ì¨²ÎÊý
    {"ÔÆÆ½Ì¨ÉÏ´«:","UPLOAD:"},//wifi ÉÏ´«
    {"±¨¾¯Êä³ö:","ALARM OUTPUT:"},//±¨¾¯Êä³ö
    {"Âö³åÒ»:","PLS1:"},//Âö³å1 Êä³ö
    {"Âö³å¶þ:","PLS2:"},//Âö³å2 Êä³ö
    {"Í¬²½´¥·¢:","SYNC TRIGGER:"},//Í¬²½´¥·¢
};    //
//°ïÖú²Ëµ¥ÏÔÊ¾
static const struct HelpStr helpS[IFNUM]=
{
    {"°ïÖú:Éè¶¨½Ó¿Ú±àºÅ¡£\0","Please set USB ID.\0"},
    {"°ïÖú:Éè¶¨ÔÆÆ½Ì¨²ÎÊý¡£\0","Please set cloud platform parameters.\0"},
    {"°ïÖú:Éè¶¨ÔÆÆ½Ì¨Êý¾ÝÉÏ´«¡£\0","Please set cloud platform data upload.\0"},
    {"°ïÖú:Éè¶¨±¨¾¯Êä³ö¡£\0","Please set cloud alarm output.\0"},
    {"°ïÖú:Éè¶¨Âö³åÒ»Êä³ö¡£\0","Please set PLS1 output.\0"},
...
{
    {{"ÈÈµãÃÜÂë\0","PassW\0"},{"ÈÈµãÃû³Æ","Name\0"}},
    {{"Ä¿±ê¶Ë¿Ú\0","DestP\0"},{"ÓòÃû","Domain\0"}},
    {{"±¾µØ¶Ë¿Ú\0","LocolP\0"},{"±¾µØ±àºÅ","ID\0"}},

};

const char FuHao[DiaSoftKeyCAM] ="-/_()<>:*#%~\\";

struct RDispStr DisWifi[DiaWifiCOL][DiaWifiCAM] = {0};//×Ó¶Ô»°¿òµçÑ¹¶Ô»°¿òµÄ¿Ø¼ü¾ØÕó

struct RDispStr DisSoftKey[DiaSoftKeyCOL][DiaSoftKeyCAM] = {0};//×Ó¶Ô»°¿òµçÑ¹¶Ô»°¿òµÄ¿Ø¼ü¾ØÕó
...
void Stu_IF_Init(void)
{
    ...
    for(m=0; m<IFCOL; m++)
    {
        ...
        DisIF[m].flag = 1;  //³õÊ¼»¯µÚÒ»ÏÔÊ¾±êÖ¾
        DisIF[m].selold = 1;//ÒÔÇ°È«Ñ¡ÖÐÏÖÔÚÈ«Ã»Ñ¡ÖÐ Ë¢ÐÂ
        DisIF[m].sel= 0;     //µ±Ç°Ã»ÓÐÑ¡ÖÐ
        DisIF[m].disp=1;  // ÊÇ·ñÏÔÊ¾1 ÏÔÊ¾ 0²»ÏÔÊ¾
        DisIF[m].dispold=1; //Ö®Ç°ÊÇ·ñÏÔÊ¾
        DisIF[m].Box.flag = 0;  //×Ö·û´®ÊÇ·ñÓÐ¿ò 0 Ã»ÓÐ¿ò
        DisIF[m].FontCH.flag = 0;  //ÖÐÎÄ±êÖ¾
        DisIF[m].FontEN.flag = 0;  //Ó¢ÎÄ±êÖ¾
        DisIF[m].cind=m;//ÀàÐÍ
        DisIF[m].Ceng= 0;
        ...
        {
            DisIF[m].Ceng=2;//×ÓÏÂÀ­¿òÓÐ¼¸¸öÑ¡Ïî 2¸ö
        }
    }
    DisIF[0].flag = 0;  //³õÊ¼»¯µÚÒ»ÏÔÊ¾±êÖ¾
    DisIF[0].sel= 1;//µ±Ç°ÎªÑ¡ÖÐ×´Ì¬
    DisIF[0].selold = 0;//ÉÏÒ»´ÎÎªÑ¡ÖÐ×´Ì¬
    for(n=0; n<HCAM; n++)
    {
        DisLog[12][n].sel= 0;
...
void Stu_IF_cycle(struct CUR *sle,struct CUR *sleold)
{
    ...
    {
        DisIF[sle->COL].selold = 0;//Ë¢ÐÂ
        DisIF[sle->COL].sel= 1;
        DisIF[sleold->COL].selold = 1;//Ë¢ÐÂ
...
void WifiDiaFlash(uint8 keytmp,struct RDispPara* RD,struct CUR* cur)
{
    ...
    DisWifi[0][0].cind =  CindStr; //ÃÜÂë
    DisWifi[0][1].cind =  CindStr ;//Ãû³Æ
    DisWifi[1][0].cind  = CindNum;//Ä¿±ê¶Ë¿Ú
    ...
    if(CHI == DataSave.Data_type.LANG )
    {
        ...
        LCD_DisplayStringLineLJ(x1y1.Box.x1+2, x1y1.Box.y1+2, "ÔÆÆ½Ì¨ÉèÖÃ",FONTTYPE16);

        LCD_SetBackColor(LCD_COLOR_BLACK);
        LCD_DisplayStringLineLJ(x1y1.Box.x1+2, x1y1.Box.y1+Ydiv, "ÈÈµãÃÜÂë",FONTTYPE16);
        LCD_DisplayStringLineLJ(x1y1.Box.x1+2+Xdiv*3+15, x1y1.Box.y1+Ydiv, "ÈÈµãÃû³Æ",FONTTYPE16);
        LCD_DisplayStringLineLJ(x1y1.Box.x1+2, x1y1.Box.y1+2+Ydiv*4, "Ä¿±ê¶Ë¿Ú",FONTTYPE16);
        LCD_DisplayStringLineLJ(x1y1.Box.x1+2+Xdiv*3+15, x1y1.Box.y1+2+Ydiv*4, "ÓòÃû",FONTTYPE16);
        LCD_DisplayStringLineLJ(x1y1.Box.x1+2, x1y1.Box.y1+2+Ydiv*7, "±¾µØ¶Ë¿Ú",FONTTYPE16);
        LCD_DisplayStringLineLJ(x1y1.Box.x1+2+Xdiv*3+15, x1y1.Box.y1+2+Ydiv*7, "±¾µØ±àºÅ",FONTTYPE16);
    }
    else if(ENG == DataSave.Data_type.LANG)
    {
        LCD_SetBackColor(LCD_COLOR_BLUE);
...
void WifiDiaInit(uint8 keytmp,struct RDispPara* RD,struct CUR* cur)
{
    ...
    DisWifi[3][1].cind=16;
    DisWifi[3][1].index[DisWifi[3][1].cind] = 1;
    strncpy(DisWifi[0][0].Str,DataSave.Data_type.parameters.Wifi_PW,sizeof(DataSave.Data_type.parameters.Wifi_PW)); //ÃÜÂë
    strncpy(DisWifi[0][1].Str,DataSave.Data_type.parameters.Wifi_Name,sizeof(DataSave.Data_type.parameters.Wifi_Name));   //Ãû³Æ
...
void ModifyWifi(uint8 keytmp,struct RDispPara* RD,struct RDispPara* RDOld,struct CUR* cur,char MaxCol,char MaxCam)
{
    ...
    switch(keytmp)
    {
        ...
        case KEY_UP://ÉÏ
            ...
            while(DisWifi[cur->COL][cur->CAM].disp!=1);  //²»ÏÔÊ¾Ìø¹ý
...
void ModifySoftKey(uint8 keytmp,struct RDispStr* RD,struct RDispStr* RDOld,struct CUR* cur,char MaxCol,char MaxCam,uint8 MaxNum)
{
    switch(keytmp)
    {
        ...
        case KEY_UP://ÉÏ
            ...
            while(DisSoftKey[cur->COL][cur->CAM].disp!=1);  //²»ÏÔÊ¾Ìø¹ý
...
static void wifiDia(uint8 keytmp,struct RDispPara* RD,struct CUR* cur)
{
    ...
    OSSchedLock(&err);
    strncpy(DataSave.Data_type.parameters.Wifi_PW,DisWifi[0][0].Str,sizeof(DataSave.Data_type.parameters.Wifi_PW)); //ÃÜÂë
    strncpy(DataSave.Data_type.parameters.Wifi_Name,DisWifi[0][1].Str,sizeof(DataSave.Data_type.parameters.Wifi_Name));  //Ãû³Æ
    tempval=Myatoi(DisWifi[1][0].Str);  //Ä¿±ê¶Ë¿Ú
    if( tempval<=0xffff&&tempval>=0x0)
        DataSave.Data_type.parameters.Ser_Port=Myatou16(DisWifi[1][0].Str);  //Ä¿±ê¶Ë¿Ú
    strncpy(DataSave.Data_type.parameters.YuMing,DisWifi[1][1].Str,sizeof(DataSave.Data_type.parameters.YuMing));
    tempval=Myatoi(DisWifi[2][0].Str);  //±¾µØ¶Ë¿Ú
    if( tempval<=0xffff&&tempval>=0x0)
        DataSave.Data_type.parameters.Port=Myatou16(DisWifi[2][0].Str);//±¾µØ¶Ë¿Ú
    tempval=Myatoi(DisWifi[2][1].Str);  //±¾µØid
    if( tempval<=0xffff&&tempval>=0x0)
        DataSave.Data_type.parameters.modbus_id=Myatou16(DisWifi[2][1].Str ); //±¾µØid
    Stu_MinIF_Init();
    ModifyWifi(keytmp,RD,&DisLogOld,&Cur,DiaWifiCOL,DiaWifiCAM);
    page_IF();
    SaveAllPara();
...
//lj  ÎªÁËÌáËÙ18ÏîÏÂÀ­²Ëµ¥°´¼üÏìÓ¦±äÂý×öÁË¸ü¸Ä
static void ComBoxData(uint8 keytmp,struct RDispPara* RD,struct CUR* cur,uint8 MaxCol)
{
    struct BoxFram  box;
    uint8 Ceng =0 ;
    uint8 m;
    struct RDispPara DisInPut[USBIDCOL] = {0};   //Ñ¡×î´óµÄ
    static struct RDispPara DisLogOld;
    box=RD->Box;
    Ceng = RD->Ceng;
    for(m=0; m<MaxCol; m++)
...
uint8 ModifyIF(uint8 keytmp,struct CUR* cur,struct CUR* curold)                 // ¼üÖµÆ¥Åäº¯Êý£¬ÔÚwhile´óÑ­»·ÖÐ·´¸´Æ¥Åä
{
    OS_ERR err;
    *curold=*cur;
    ...
    switch(cur->OPN)
    {
        ...
        case KEY_UP://ÉÏ
            ...
            break;
        case KEY_DOWN:
            if(cur->COL<IFCOL-1) //×îºóÒ»ÐÐÊÇ¹¦ÄÜ¼ü
                cur->COL++;
            else
                cur->COL=0;
            break;
        ...
        case KEY_ENTER:
            ...
            else if(cur->COL==2)
            {
                ...
                if(DataSave.Data_type.parameters.SW!= DisIF[cur->COL].index[DisIF[cur->COL].cind])
                {
                    ...
                    {
                        /* ´´½¨WIFIÈÎÎñ Ê±¼äË¢ÐÂ1ms Ò»´Î */
                        OSTaskCreate((OS_TCB     *)&WIFICheckTaskTCB,                             //ÈÎÎñ¿ØÖÆ¿éµØÖ·
                                     (CPU_CHAR   *)"WIFICheck Task",                             //ÈÎÎñÃû³Æ
                                     (OS_TASK_PTR ) WIFICheck_task,                                //ÈÎÎñº¯Êý
                                     (void       *) 0,                                          //´«µÝ¸øÈÎÎñº¯Êý£¨ÐÎ²Îp_arg£©µÄÊµ²Î
                                     (OS_PRIO     ) WIFICheck_TASK_PRIO,                          //ÈÎÎñµÄÓÅÏÈ¼¶
                                     (CPU_STK    *)&WIFICheck_TASK_STK[0],                          //ÈÎÎñ¶ÑÕ»µÄ»ùµØÖ·
                                     (CPU_STK_SIZE) WIFICheck_STK_SIZE / 10,                //ÈÎÎñ¶ÑÕ»¿Õ¼äÊ£ÏÂ1/10Ê±ÏÞÖÆÆäÔö³¤
                                     (CPU_STK_SIZE) WIFICheck_STK_SIZE,                     //ÈÎÎñ¶ÑÕ»¿Õ¼ä£¨µ¥Î»£ºsizeof(CPU_STK)£©
                                     (OS_MSG_QTY  ) 1u,                                         //ÈÎÎñ¿É½ÓÊÕµÄ×î´óÏûÏ¢Êý
                                     ...
                                     (OS_ERR     *)&err);
                        /* ´´½¨WIFIÈÎÎñ Ê±¼ä2s ÉÏ´«Ò»´Î */
                        OSTaskCreate((OS_TCB     *)&WIFITaskTCB,                             //ÈÎÎñ¿ØÖÆ¿éµØÖ·
                                     (CPU_CHAR   *)"WIFI Task",                             //ÈÎÎñÃû³Æ
                                     (OS_TASK_PTR ) WIFI_task,                                //ÈÎÎñº¯Êý
                                     (void       *) 0,                                          //´«µÝ¸øÈÎÎñº¯Êý£¨ÐÎ²Îp_arg£©µÄÊµ²Î
                                     (OS_PRIO     ) WIFI_TASK_PRIO,                          //ÈÎÎñµÄÓÅÏÈ¼¶
                                     (CPU_STK    *)&WIFI_TASK_STK[0],                          //ÈÎÎñ¶ÑÕ»µÄ»ùµØÖ·
                                     (CPU_STK_SIZE) WIFI_STK_SIZE / 10,                //ÈÎÎñ¶ÑÕ»¿Õ¼äÊ£ÏÂ1/10Ê±ÏÞÖÆÆäÔö³¤
                                     (CPU_STK_SIZE) WIFI_STK_SIZE,                     //ÈÎÎñ¶ÑÕ»¿Õ¼ä£¨µ¥Î»£ºsizeof(CPU_STK)£©
                                     (OS_MSG_QTY  ) 1u,                                         //ÈÎÎñ¿É½ÓÊÕµÄ×î´óÏûÏ¢Êý
                                     (OS_TICK     ) 0u,                                         //ÈÎÎñµÄÊ±¼äÆ¬½ÚÅÄÊý£¨0±íÄ¬ÈÏÖµOSCfg_TickRate_Hz/10£©
                                     (void       *) 0,                                          //ÈÎÎñÀ©Õ¹£¨0±í²»À©Õ¹£©
                                     (OS_OPT      )(OS_OPT_TASK_STK_CHK | OS_OPT_TASK_STK_CLR), //ÈÎÎñÑ¡Ïî
            ...
            else if(cur->COL==6)
            {
                ...
                if(DataSave.Data_type.SYNF != DisIF[cur->COL].index[DisIF[cur->COL].cind])
                {
                    ...
                    OSTaskSuspend((OS_TCB*)&DoUartTaskTCB,&err);//¹ÒÆðÈÎÎñ
                    ChanSet.SYNC = DataSave.Data_type.SYNF+1;
                    SendSetSYNCTime(ChanSet);
                    ChanSet.SYNC = 0;
                    OSSemPend(&Uart_CMD,100,OS_OPT_PEND_BLOCKING,0,&err); //ÇëÇóÐÅºÅÁ¿¹ÒÆð
                    SaveAllPara();
...
//        BluidICOTask();
     ...
     //   OSTaskDel(&ICOTaskTCB,&err);
        keytmp=ModifyIF(keytmp,&CurIF,&CurIFOld);     //´¦Àí¼üÖµ
