...
void SPI_FLASH_Init(void)
{
 ...
 // RCC_AHB1PeriphClockCmd (FLASH_SPI_SCK_GPIO_CLK | FLASH_SPI_MISO_GPIO_CLK|FLASH_SPI_MOSI_GPIO_CLK|FLASH_CS_GPIO_CLK, ENABLE);
  FLASH_SPI_CLK_ENABLE();              
	FLASH_SPI_SCK_ENABLE();     
	FLASH_SPI_MISO_ENABLE();              
	FLASH_SPI_MOSI_ENABLE();      
	FLASH_CS_GPIO_ENABLE();  
	
  /*!< SPI_FLASH_SPI 时钟使能 */
 // FLASH_SPI_CLK_INIT(FLASH_SPI_CLK, ENABLE);
 
  //设置引脚复用
	 GPIO_InitStructure.Pin = FLASH_SPI_SCK_PIN|FLASH_SPI_MISO_PIN;
	 GPIO_InitStructure.Mode = GPIO_MODE_AF_PP;
	 GPIO_InitStructure.Speed = GPIO_SPEED_FREQ_HIGH;
	 HAL_GPIO_Init(FLASH_SPI_SCK_GPIO_PORT, &GPIO_InitStructure);
	
	 GPIO_InitStructure.Pin = FLASH_SPI_MISO_PIN;
	 GPIO_InitStructure.Mode = GPIO_MODE_INPUT;
	 GPIO_InitStructure.Pull = GPIO_NOPULL;
	 HAL_GPIO_Init(FLASH_SPI_MISO_GPIO_PORT, &GPIO_InitStructure);
