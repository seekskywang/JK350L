...
static void LL_ConvertLineToARGB8888(void * pSrc, void *pDst, uint32_t xSize, uint32_t ColorMode);
 ...
 void Pin_GPIO_Config_Full(GPIO_TypeDef  *GPIOx,uint32_t Pin,uint32_t Mode,uint32_t Pull,uint32_t Speed)
{	
...
//  BEEP_GPIO_CLK; 
  ...
  GPIO_InitStruct.Pin = LCD_PWM_GPIO_PIN;
...
//  HAL_GPIO_Init(BEEP_GPIO_PORT, &GPIO_InitStruct);
  ...
  // 开启LCD相关的GPIO外设时钟
  LTDC_R2_GPIO_CLK; LTDC_R3_GPIO_CLK; LTDC_R4_GPIO_CLK;
  LTDC_R5_GPIO_CLK; LTDC_R6_GPIO_CLK; LTDC_R7_GPIO_CLK;
  LTDC_G2_GPIO_CLK; LTDC_G3_GPIO_CLK; LTDC_G4_GPIO_CLK;
  LTDC_G5_GPIO_CLK; LTDC_G6_GPIO_CLK; LTDC_G7_GPIO_CLK;
  LTDC_B2_GPIO_CLK; LTDC_B3_GPIO_CLK; LTDC_B4_GPIO_CLK;
  LTDC_B5_GPIO_CLK; LTDC_B6_GPIO_CLK; LTDC_B7_GPIO_CLK;
...
////RGB888
////	  periph_clk_init_struct.PLLSAI.PLLSAIN = 420;
////periph_clk_init_struct.PLLSAI.PLLSAIN = 400;
// //periph_clk_init_struct.PLLSAI.PLLSAIQ = 20;
//  //  periph_clk_init_struct.PLLSAI.PLLSAIR = 26;
//  //periph_clk_init_struct.PLLSAI.PLLSAIR = 2;
//   //periph_clk_init_struct.PLLSAIDivR = RCC_PLLSAIDIVR_8;
//		
//		
//		
//   periph_clk_init_struct.PLLSAI.PLLSAIN = 304;
//  //periph_clk_init_struct.PLLSAI.PLLSAIN = 400;
//   periph_clk_init_struct.PLLSAI.PLLSAIQ = 7;
...
void LCD_Init(void) {
  ...
  Ltdc_Handler.Instance = LTDC;     // 配置LTDC参数
  // 配置行同步信号宽度(HSW - 1)
  Ltdc_Handler.Init.HorizontalSync = HSW - 1;
  // 配置垂直同步信号宽度(VSW - 1)
  Ltdc_Handler.Init.VerticalSync = VSW - 1;
  ...
  // PLLLCDCLK = PLLSAI_VCO Output/PLLSAIR = 192/5 = 38.4 Mhz
  // LTDC clock frequency = PLLLCDCLK / LTDC_PLLSAI_DIVR_4 = 38.4 / 4 = 9.6Mhz
  // 当前配置为25.2M频率,对应800x525x60hz刷新率
  // 因为没有用过767,没有仔细研究过怎样的分频最合理,所以下列参数是简单凑出来的,望改进
  periph_clk_init_struct.PeriphClockSelection = RCC_PERIPHCLK_LTDC;
  periph_clk_init_struct.PLLSAI.PLLSAIN = 252;
  periph_clk_init_struct.PLLSAI.PLLSAIR = 5;
...
uint16_t RGBTo16RGB(uint32_t RGB_Code)
{
	...
	uint16_t B;
	uint16_t temp;
	B = ((RGB_Code&0xFF)*0x1F)/0xFF;
	G = (((RGB_Code>>8)&0xFF)*0x3F)/0xFF;
	R = (((RGB_Code>>16)&0xFF)*0x1F)/0xFF;
	
//	B = (((RGB_Code&0xFF)*0x76)>>2)/0xFF;
//	G = ((((RGB_Code>>8)&0xFF)*0xF6)>>2)/0xFF;
//	R = ((((RGB_Code>>16)&0xFF)*0x76)>>2)/0xFF;
	temp = (R<<11)|(G<<5)|B;
	return temp;
}
/**
...
void LCD_DrawPixel(uint16_t Xpos, uint16_t Ypos, uint32_t RGB_Code)
{
  ...
  else if(Ltdc_Handler.LayerCfg[ActiveLayer].PixelFormat == LTDC_PIXEL_FORMAT_RGB888)
  {
	...
	*(__IO uint8_t*) (Ltdc_Handler.LayerCfg[ActiveLayer].FBStartAdress + (3*(Ypos*LCD_GetXSize() + Xpos))) = 0xFF&RGB_Code;
          ...
          (Ltdc_Handler.LayerCfg[ActiveLayer].PixelFormat == LTDC_PIXEL_FORMAT_AL88))  
  {
    *(__IO uint16_t*) (Ltdc_Handler.LayerCfg[ActiveLayer].FBStartAdress + (2*(Ypos*LCD_GetXSize() + Xpos))) = RGBTo16RGB(RGB_Code);   
  }
  else
  {
...
void LCD_DrawPixel_LJ(uint16_t Xpos, uint16_t Ypos, uint16_t RGB_Code)
{
    ...
    *(__IO uint16_t*) (Ltdc_Handler.LayerCfg[ActiveLayer].FBStartAdress + (2*(Ypos*LCD_GetXSize() + Xpos))) = RGB_Code;   
...
void DrawChar_16(uint16_t Xpos, uint16_t Ypos, const uint8_t *c)
{
  uint32_t i = 0, j = 0;
  uint16_t height, width;
  uint8_t  offset;
...
//	OSSchedLock(&err);
  ...
  while ((refcolumn < LCD_PIXEL_WIDTH) && ((*ptr != 0) & (((refcolumn + DrawProp[ActiveLayer].pFont->Width) & 0xFFFF) >= DrawProp[ActiveLayer].pFont->Width)))
  {
	...
	if ( * ptr <= 126 )	           	//英文字符
	{
		...
		LCD_DisplayChar(refcolumn, refline, *ptr);
		/* 根据字体偏移显示的位置 */
...
static void LL_FillBuffer(uint32_t LayerIndex, void *pDst, uint32_t xSize, uint32_t ySize, uint32_t OffLine, uint32_t ColorIndex) 
{
  ...
  { 
    Dma2d_Handler.Init.ColorMode    = DMA2D_RGB565;
		//lj为适应565
	//	ColorIndex = RGBTo16RGB(ColorIndex);
  }
	...
	{   
		...
		for(t1=0;t1<8;t1++)
		{
			if(temp&0x80)
				...
				LCD_DrawPixel(x,y, DrawProp[ActiveLayer].BackColor); 			 
